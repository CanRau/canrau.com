...
![The og:image of this article citing the title, my GitHub avatar picture and this sites domain](/assets/images/og.png?v=6&size=small&rev=i3_KGR9q48&slug=dynamic-social-images-skia-remix)
...
In dev mode I need a quick feedback loop so I decided to set the `Content-Disposition` header to `inline` so that the image shows directly in the browser. In production it'll be set to `attachment` which indicates the browser to download the image with the specified name.
